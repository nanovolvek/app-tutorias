...
from app.models.prueba_unidad import PruebaUnidadEstudiante, PorcentajeLogro
...
# Esquemas Pydantic
class PruebaUnidadUpdateRequest(BaseModel):
    student_id: int
    unidad: str
    modulo: str
    resultado: str
...
def get_students_pruebas(
    ...
    """Obtener pruebas de unidad de estudiantes con filtros"""
    try:
        if not unidad:
        ...
        for estudiante in estudiantes:
            ...
            pruebas_existentes = db.query(PruebaUnidadEstudiante).filter(
...
def get_all_pruebas_for_export(
    ...
    try:
        ...
        pruebas_map = {}
        all_pruebas = db.query(PruebaUnidadEstudiante).all()
...
def update_student_prueba(
    request: PruebaUnidadUpdateRequest,
    db: Session = Depends(get_db),
    current_user = Depends(get_current_user)
):
    """Crear o actualizar prueba de unidad de estudiante"""
    ...
    existing_record = db.query(PruebaUnidadEstudiante).filter(
        PruebaUnidadEstudiante.estudiante_id == request.student_id,
        PruebaUnidadEstudiante.unidad == request.unidad,
        PruebaUnidadEstudiante.modulo == request.modulo
    ).first()
    ...
    if existing_record:
        ...
        new_record = PruebaUnidadEstudiante(
