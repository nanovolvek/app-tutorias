...
const PruebaDiagnostico: React.FC = () => {
  ...
  const fetchUnidades = async () => {
    try {
      const response = await fetchWithAuth('/prueba-diagnostico/unidades');
      if (response.ok) {
  ...
  const fetchEquipos = async () => {
    try {
      const response = await fetchWithAuth('/prueba-diagnostico/equipos');
      if (response.ok) {
        const data = await response.json();
        setEquipos(data);
      }
  ...
  const fetchPruebasData = async () => {
    ...
    try {
      ...
      const url = `/prueba-diagnostico/students?${params.toString()}`;
  ...
  const updatePruebaStatus = async (studentId: number, modulo: string, newStatus: string) => {
    try {
      const response = await fetchWithAuth('/prueba-diagnostico/students', {
        method: 'POST',
        body: JSON.stringify({
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const response = await fetchWithAuth('/prueba-diagnostico/export-all');
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Prueba Diagnóstico');
      
      // Descargar archivo
      const fileName = `prueba_diagnostico_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    <div className="page-container">
      <div className="tickets-header">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
          <div>
            <h1 className="page-title">📊 Prueba Diagnóstico</h1>
            <p className="page-description">
              Sistema de seguimiento de pruebas de diagnóstico por unidad y módulo. 
      ...
      {!loading && selectedUnidad && (
        <div className="tickets-content">
          <h2>Pruebas de Diagnóstico - {getUnidadNombre()}</h2>
...
export default PruebaDiagnostico;
